...
fn main() {
    ...
    let active = Topic::new(
        "active",
        MessageField::Var(Variable::new_boolean("data", VariableType::Measured)),
    );
    let activate = Topic::new(
        "activate",
        MessageField::Var(Variable::new_boolean("data", VariableType::Command)),
    );
    ...
    let r1 = m
        .add_item(SPItem::Resource(r1))
        .global_path()
        .clone()
        .unwrap();

    let resource = if let Some(SPItemRef::Resource(r)) = m.get(&r1.to_sp()) {
        Some(r)
    } else {
        None
    };
